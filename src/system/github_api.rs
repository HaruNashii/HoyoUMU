...
lazy_static! { pub static ref GITHUB_API_TIME_RESET: Mutex<Option<String>> = Mutex::new(None); }
...
pub async fn fetch_github_api(url: &str) -> Result<Value, Box<dyn std::error::Error>>
{
    ...
    if let Some(remaining) = res.headers().get("x-ratelimit-remaining") && remaining.to_str().unwrap().parse::<i32>().unwrap() <= 10
    {
...
pub async fn check_if_github_api_is_available() -> bool
{
    ...
    match fetch_github_api(url).await
    {
        ...
        Err(err) =>
        {
            if err.to_string().contains("Rate limit exceeded") { return false };
